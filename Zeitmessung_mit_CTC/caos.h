...
//////////////////////////
// Modulsteuerung
#define modu_get(x)   ( modu( 1, x, 0))
#define modu_set(x,y) ( modu( 2, x, y))

//////////////////////////
// KC85 IRM definitions
#define IRM_CURSO       *(volatile uint16_t*) 0xB7A0
#define IRM_CURSOX      *(volatile uint8_t*)  0xB7A0
#define IRM_CURSOY      *(volatile uint8_t*)  0xB7A1
#define IRM_COLOR       *(volatile uint8_t*)  0xB7A3
#define IRM_CCTL0       *(volatile uint16_t*) 0xB7A6
#define IRM_CCTL1       *(volatile uint16_t*) 0xB7A8
#define IRM_CCTL2       *(volatile uint16_t*) 0xB7AA
#define IRM_CCTL3       *(volatile uint16_t*) 0xB7AC
#define BASIC_MENU      *(volatile uint8_t*)  0xE011    // =0x7F bei KC85/4
#define CAOS_VERSION    *(volatile uint8_t*)  0xEDFF    // BCD, ab 4.1
 ...
 *      no keypress: A = 0
 */
extern unsigned char kbds( void);


/*
 * Sprung auf RESET (Warmstart des Systems)
 */
extern void bye( void) __naked; // no return
 ...
 *      no keypress: A = 0
 */
extern unsigned char kbdz( void);
 ...
 */
extern void loop( void) __naked; // no return
 ...
 */
extern void jump( unsigned char modulsteckplatz) __naked;
 ...
 * Achtung, hier nur Pixelposition!
 */
extern unsigned char * padr( unsigned char row, unsigned char column);
